...
- `INPAINT_ENABLED`, `INPAINT_ENGINE`, and the remaining `BUBBLE_`, `TEXT_DARK_`, `WHITE_BACKGROUND_`, `INPAINT_`, `MASK_`, and `OCR_CLEAR_PADDING` settings: Source-text removal behavior. `OCR_CLEAR_PADDING` defaults to `12` pixels and expands the detection area used for clearing while the bubble mask still limits the cleared pixels.
